...
This directory contains the CI pipeline for `keystate-cli`. There is one
workflow, `ci.yml`; it runs on the same GNU/Linux runner (`ubuntu-latest`)
with the stable Rust toolchain and shares the caching from
`Swatinem/rust-cache`. It mirrors the `keystate-core` and
`keystate-adapter-keycloak` pipelines.
...
binary and Docker image on GitHub Releases (see `RELEASE.md`), so there is no
...
- `GITHUB_TOKEN` — built-in, used by the audit and deny jobs.
